 ...
 * 값은 **기기 밖으로 나가지 않는다**(계측·백업 페이로드·서버 전송 없음). 게이트 판정(`docs/ALPHA.md`)이
 * 테스터의 기억 대신 이 숫자를 읽는다.
 */
class AppOpenTracker(context: Context) {
    ...
    fun recordOpen(todayEpochDay: Long = LocalDate.now().toEpochDay()) {
        // 보관 한도를 넘은 날짜는 record가 걸러낸다 — 저장값이 무한 증가하지 않는다
        val days = OpenDays.record(storedDays(), todayEpochDay)
        prefs
            .edit()
            .putLong(KEY_LAST_OPEN, todayEpochDay)
