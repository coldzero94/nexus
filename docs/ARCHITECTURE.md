...
├── kotlin/                     # Kotlin Gradle 루트 · Kotlin 2.4.x · AGP 9.x (앱+서버 공존 — Confetti 패턴)
│   ├── build-logic/            #   컨벤션 플러그인 (nexus.kmp.library 등, includeBuild)
│   ├── gradle/libs.versions.toml
│   ├── core/                   #   KMP 라이브러리 — XP 엔진·상태머신·모델 (commonMain)
│   │                           #   · android + iosArm64(§2) + jvm(서버용) 타깃
│   ├── app/                    #   Android 앱 진입점 — Compose·Health Connect·Room·Glance 조립
│   └── server/                 #   (S9~) Ktor 3.5 — core 직접 의존, 활동 요약 검증 API
│                               #   멀티스테이지 Dockerfile → Cloud Run 서울 (무료·scale-to-zero)
├── iosApp/                     # 게이트 후 생성 — 지금은 디렉터리도 만들지 않음
├── supabase/                   # (S9~) config.toml · migrations/ · seed.sql (Edge Functions 불채택)
...
- 각 파트는 자기 도구를 유지: `kotlin/`=Gradle(앱+서버), `web/`=pnpm(Node), `supabase/`=마이그레이션 중심(supabase CLI). Edge Functions는 불채택([BACKEND.md §4](./BACKEND.md) — 산식 이원화 방지, 계산은 `kotlin/server/`).
...
## 5. Supabase-in-repo (S9~) — DB·Auth 전용

- 루트 `supabase/` 디렉토리(공식 표준): config.toml·migrations/·seed.sql. **Edge Functions 불채택** — 계산·검증은 `kotlin/server/`(Ktor)가 담당(산식 단일 진실). 로컬 루프 = `supabase start`(Docker) → `db reset`.
- **마이그레이션 규율**: 모든 스키마 변경은 `supabase migration new`로만, 대시보드 직접 수정 금지(드리프트 시 CI가 깨짐). PR CI = 로컬 스택에서 `db reset` 검증 + `gen types typescript --local` 후 `git diff --exit-code`(타입 드리프트 검출 — 웹 공유 페이지용이자 'Kotlin 모델 갱신' 신호). main 머지 = `db push` + server 이미지 Cloud Run 배포.
- **서버↔DB**: Ktor는 Supavisor 풀러(session 5432) 경유 JDBC, 인스턴스당 풀 3~5(무료 티어 연결 한도), 서버 경로는 RLS 우회라 **JWT sub 스코핑 계층 강제**. 인증은 JWKS 비대칭 키, 신 `sb_secret`/`sb_publishable` 키 체계(legacy는 2026년 말 폐기). 시크릿은 배포 플랫폼 시크릿으로만.
...
| Supabase (DB·Auth) | Free 티어 + keep-alive cron. 실사용자 시점에 Pro $25/월 |
| 서버 컴퓨트 (S9~) | **Cloud Run 서울 무료** (월 200만 요청·scale-to-zero, 카드 등록 필요 — max-instances 상한+예산 알림으로 과금 차단) |
