...
**이 4가지는 이제 코드가 스스로 검사한다 (#245, 2026-07-30)**: `core/LedgerIntegrity.check(rows)`가 위 계약을 런타임 불변식으로 판정한다 — 시퀀스 단조(계약 3), 중복 GRANT(계약 1의 멱등성이 실제로 지켜졌는가), 취소↔지급 짝·부호·**지급일 보존**(계약 4), 같은 날 formulaVersion 혼재, 저장 총합 재계산 불일치. 계약을 문서에만 적어두고 지켜지는지 확인하는 코드가 없던 것이 가장 큰 구멍이었다 — 원장은 append-only라 오염을 되돌릴 수 없고, S9 서버 임포트 때 발견하면 이미 몇 주치가 굳어 있다.

가장 미묘한 위반은 `CANCELLATION_DAY_MISMATCH`다. 취소 행의 `xp`가 지급의 정확한 반대값이어도 **지급일이 아닌 다른 날에 들어가면** 상쇄가 되지 않는다 — 상한이 일 단위 합에 걸리므로 지급일은 상한이 그대로 남고 취소일만 음수가 되어 0으로 클램프된다. 결과는 "삭제한 활동의 XP가 그대로 남음"이고, 이벤트만 눈으로 봐서는 정상으로 보인다.

**자동 수정은 하지 않는다.** 위반을 발견해도 행을 지우거나 보정 이벤트를 넣지 않는다 — append-only가 이 원장의 유일한 안전 장치이고, "고치는 코드"가 곧 다음 오염의 경로다. 대응은 호출 지점별로 갈린다(`app/diag/LedgerIntegrityGuard`): 앱 시작·동기화 같은 자동 경로는 **보고만**(E-레벨 로그 + `FailureCategory.LEDGER_INTEGRITY` + 로컬 실패 카운터), 크래시는 개발자가 디버그 도구에서 명시적으로 요청할 때만. 자동 경로에서 던지면 동기화는 `Result.retry()`로 번역되어 조용한 무한 재시도가 되고, 시작 경로에서는 디버그 시더로 심은 위반을 되돌릴 화면에 닿을 수 없게 된다.
...
- **사용자 식별은 소셜 로그인 전제(Google 우선, iOS 확장 시 Apple 추가)** — MVP는 계정 없음(로컬 UUID)이지만 나중에 연결 가능한 구조로. 어느 백엔드로든 이식됨.
  - **구현(#240, 2026-07-30)**: `data/InstallIdStore` — 첫 호출에 v4 UUID 1회 발급·prefs 영속. 백업 스냅샷(`BackupSnapshot.installId`, v2)에 포함하고 **복원 시 갓 생성된 로컬 값을 대체**한다(복원의 목적이 "이전 설치의 연속"이므로). 이 앵커가 없으면 #80 계정 연결의 "기존 캐릭터·원장 무손실 승계"가 성립하지 않는다 — 재설치·기기 이전 후 새 UUID를 받으면 이전 원장의 소유자를 알 수 없고, §1이 경고하는 "나중에 추가하면 소급 보강 불가"에 정확히 해당한다. 계측·크래시 페이로드에는 싣지 않는다(불변식 ②).
