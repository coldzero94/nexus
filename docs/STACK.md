...
- **렌더 낭비 3원칙 (#246)** — 저사양 갤럭시 A가 알파 base라 상시 낭비가 곧 배터리다.
  ...
  ② 티커는 화면이 보일 때만 돈다(lifecycle ≥ RESUMED). 리듀스드모션(#228)과 **별개 트리거**라 한쪽이 꺼져도 다른 쪽은 그대로 작동해야 한다. 실측: 백그라운드 15초당 CPU 36 jiffies → 0.
  ③ 위젯은 **그릴 게 바뀐 때만** 갱신한다. 판정 키에 스냅샷뿐 아니라 *시각 파생 표시*(원정 잔여)를 함께 넣는다 — 스냅샷만 비교하면 진행 중 원정이 "약 N시간 남음"에 얼어붙고 개봉 대기 전환을 못 알린다.
